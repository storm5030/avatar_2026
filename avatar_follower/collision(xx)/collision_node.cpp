...
//   // 충돌 쌍 구성 (링크 이름도 URDF에 맞게 수정 필요)
 
//   // 팔 자가 충돌

//   std::vector<std::pair<std::string, std::string>> base_pairs = {
//     {"link1_1", "link1_6"}, {"link1_2", "link1_6"}, {"link1_3", "link1_6"}, {"link1_4", "link1_6"},
//     {"link1_1", "link1_5"}, {"link1_2", "link1_5"}, {"link1_3", "link1_5"},
//     {"link1_1", "link1_4"}, {"link1_2", "link1_4"},
//     {"link1_1", "link1_3"}
//   };

//   for (std::string side : {"left_", "right_"}) {
//       for (const auto& pair : base_pairs) {
//           collision_pairs_.push_back({side + pair.first, side + pair.second});
//       }
//   }
...
//             "left_link1_" + std::to_string(i), 
//             "right_link1_" + std::to_string(j)
//         });
//     }
//   }
//   collision_pairs_.push_back({"left_link_gripper_1","right_link_gripper_1"});

//   // 각 팔 vs 몸체 
//   for (int i = 3; i <= 6; ++i) {
//       collision_pairs_.push_back({"left_link1_" + std::to_string(i), "base_link"});
//       collision_pairs_.push_back({"right_link1_" + std::to_string(i), "base_link"});
//   }
//   collision_pairs_.push_back({"left_link_gripper_1", "base_link"});
//   collision_pairs_.push_back({"right_link_gripper_1", "base_link"});
...
//   for (const auto & seg : left_kdl_chain_.segments) {
...
//   all_links.insert(neck_link1_1);
//   all_links.insert(neck_link2_1);
...
// {
//   KDL::JntArray left_joint_positions(left_kdl_chain_.getNrOfJoints());
//   size_t joint_idx = 0;
//   for (const auto & seg : left_kdl_chain_.segments) {
//     //kdl_chain에서 관절 이름을 가져옴
//     const auto & joint = seg.getJoint();

//     if (joint.getType() == KDL::Joint::None) {continue;}
//     //ros메세지에서 그 관절 이름이 어디 있는지 찾음
//     auto it = std::find(msg->name.begin(), msg->name.end(), joint.getName());

//     // 찾았으면 그 위치의 값을 joint_positions에 넣음  
//     if (it != msg->name.end()) {
//       left_joint_positions(joint_idx++) = msg->position[std::distance(msg->name.begin(), it)];
//     }
//   }
